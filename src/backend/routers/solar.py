from dependencies import get_solar_fr_store, get_solar_uk_store, get_solar_de_store
...
@router.get("/solar/germany", response_model=SolarResponse)
@limiter.limit("60/minute")
async def get_solar_germany_data(
    request: Request, solar_de_store: CacheStore = Depends(get_solar_de_store)
):
    data = await solar_de_store.get()
    if data is None:
        raise HTTPException(
            status_code=503,
