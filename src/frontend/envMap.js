...
export async function initEnvMap () {
  ...
  try {
    ...
    envMapInstance.on('load', async () => {
      ...
      const ea_response = await fetch(
        'http://127.0.0.1:8000/api/environment/river_levels'
      )
      const ea_data = await ea_response.json()

      document.getElementById(
        'env-status-text'
      ).innerText = `Live / Connected (${ea_data.data.length} stations)`
      document
            ...
            .setHTML(
              `
                        <div>
                            ...
                            <p style="margin:2px 0 8px 0; color:var(--text-muted); font-size:0.75rem;">${
                            ...
                            <div style="display:flex; justify-content:space-between; margin-bottom:4px; gap: 12px">
                                ...
                                <span style="color:var(--text-main); font-size:0.875rem;">${props.value.toFixed(
                                  2
                                )} m</span>
                            </div>
                            <div style="display:flex; justify-content:space-between; gap: 12px;">
                                <span style="color:var(--text-muted); font-size:0.875rem;">Typical Range:</span>
                                <span style="color:var(--text-muted); font-size:0.875rem;">${props.low.toFixed(
                                  2
                                )} - ${props.high.toFixed(2)} m</span>
                            </div>
                        </div>
                    `
            )
